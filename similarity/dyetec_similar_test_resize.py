...
def run(anchor_image, img_rows):
    ...
    ##### model 경로 지정
    if not cuda:
        model_info = torch.load('./model_1.pt', map_location='cpu')
    else:
        model_info = torch.load('./model_1.pt')
    ...
    if not cuda:
        model_info = torch.load('./model_2.pt', map_location='cpu')
    else:
        model_info = torch.load('./model_2.pt')
    # Instantiate model
